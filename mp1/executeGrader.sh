...
SINGLE_FAILURE_GRADER_PART_ID="b9m9h"
MULTI_FAILURE_GRADER_PART_ID="MxUat"
MSG_DROP_GRADER_PART_ID="8ATm3"
...
if [ "$PARTID" == "$SINGLE_FAILURE_GRADER_PART_ID" ]; then
  #GRADER_DIRECTORY=FactoringGrader
  #SUBMISSION_CLASS=Factoring
  sh singlefailure_grader.sh /shared/submission/*
elif [ "$PARTID" == "$MULTI_FAILURE_GRADER_PART_ID" ]; then
  #GRADER_DIRECTORY=PrimeGrader
  #SUBMISSION_CLASS=Prime
  sh multifailure_grader.sh /shared/submission/*
elif [ "$PARTID" == "$MSG_DROP_GRADER_PART_ID" ]; then
  #GRADER_DIRECTORY=PrimeGrader
  #SUBMISSION_CLASS=Prime
  sh msgdropsinglefailure_grader.sh /shared/submission/*
else
